...
def check(new_f, org_f):
    new_f = new_f.applymap(lambda x: '%.4g' % x)
    org_f = org_f.applymap(lambda x: '%.4g' % x)
...
def main():
    ...
    # update
    his_overnightsmart20_RC = ff.read('overnightsmart20_RC').to_dict()
    new_overnightsmart20_RC = (overnightsmart20.shift(1,axis = 1) * ff.filter0).iloc[:,-update_win:].to_dict()
    his_overnightsmart20_RC.update(new_overnightsmart20_RC)
    his_overnightsmart20_RC = pd.DataFrame(his_overnightsmart20_RC)

    his_CTR_RC = ff.read('CTR_RC').to_dict()
    new_CTR_RC = (CTR_neu.shift(1,axis = 1) * ff.filter0).iloc[:,-update_win:].to_dict()
    # new_CTR_RC = (CTR.shift(1,axis = 1) * ff.filter0).iloc[:,-update_win:].to_dict()
    his_CTR_RC.update(new_CTR_RC)
    his_CTR_RC = pd.DataFrame(his_CTR_RC)

    his_jumpCTR_RC = ff.read('jumpCTR_RC').to_dict()
    new_jumpCTR_RC = (jumpCTR_neu.shift(1,axis = 1) * ff.filter0).iloc[:,-update_win:].to_dict()
    # new_jumpCTR_RC = (jumpCTR.shift(1,axis = 1) * ff.filter0).iloc[:,-update_win:].to_dict()
    his_jumpCTR_RC.update(new_jumpCTR_RC)
    his_jumpCTR_RC = pd.DataFrame(his_jumpCTR_RC)    
    # check
    assert check(ff.read('overnightsmart20_RC'),his_overnightsmart20_RC)
    version_reserve(his_overnightsmart20_RC,'overnightsmart20_RC') # 旧版本保留
    ff.save('overnightsmart20_RC',his_overnightsmart20_RC)
    
    assert check(ff.read('CTR_RC'),his_CTR_RC)
    version_reserve(his_CTR_RC,'CTR_RC') # 旧版本保留
    ff.save('CTR_RC',his_CTR_RC)
    
    assert check(ff.read('jumpCTR_RC'),his_jumpCTR_RC)
    version_reserve(his_jumpCTR_RC,'jumpCTR_RC') # 旧版本保留
    ff.save('jumpCTR_RC',his_jumpCTR_RC)
